...
resource "newrelic_alert_channel" "alert_channel" {
  ...
  config {
    ...
    auth_type      = each.value.channel_type == "webhook" ? lookup(each.value, "webhook_auth_type", null) : null
    auth_username  = each.value.channel_type == "webhook" ? lookup(each.value, "webhook_auth_username", null) : null
    auth_password  = each.value.channel_type == "webhook" ? lookup(each.value, "webhook_auth_password", null) : null
    headers        = each.value.channel_type == "webhook" && lookup(each.value, "webhook_headers_string", null) == null ? lookup(each.value, "webhook_headers", null) : null
    headers_string = each.value.channel_type == "webhook" && lookup(each.value, "webhook_headers", null) == null ? lookup(each.value, "webhook_headers_string", null) : null
    payload_type   = each.value.channel_type == "webhook" ? lookup(each.value, "webhook_payload_type", "application/json") : null
    payload        = each.value.channel_type == "webhook" && lookup(each.value, "webhook_payload_string", null) == null ? lookup(each.value, "webhook_payload", null) : null
    payload_string = each.value.channel_type == "webhook" && lookup(each.value, "webhook_payload", null) == null ? lookup(each.value, "webhook_payload_string", var.webhook_payload_string) : null
    ### slack config ###
